...
--http-log-path=/data/logs/nginx/logs/access.log \
...
case $version in 
1)tar fvx nginx-1.12.0.tar.gz && cd ${downloaddir}/nginx-1.12.0 && $CONFIGURE && make -j$cpunum>/dev/null && make install >/dev/null
[ $? -eq 0 ]  && echo "nginx  install  compleate,go next step." || echo "nginx install failed ,please check" 
;;
2)tar fvx nginx-1.13.2.tar.gz && cd ${downloaddir}/nginx-1.13.2 && $CONFIGURE && make -j$cpunum>/dev/null && make install >/dev/null     
[ $? -eq 0 ]  && echo "nginx  install  compleate,go next step." || echo "nginx install failed ,please check"
;;
*)tar zxf tengine-2.2.0.tar.gz   && cd ${downloaddir}/tengine-2.2.0 && $CONFIGURE && make -j$cpunum>/dev/null && make install >/dev/null
